...
#define RUNTIME_ASSERTW(x, msg)	if (!(x)) \
								{ \
									std::wcerr << "[RUNTIME_ASSERT] Assert failed. \n"; \
									std::wcerr << "[RUNTIME_ASSERT] Condition : " << #x << '\n'; \
									std::wcerr << "[RUNTIME_ASSERT] File : " << __FILE__ << '\n'; \
									std::wcerr << "[RUNTIME_ASSERT] Line : " << __LINE__ << '\n'; \
									DEBUG_ERRORW(msg, 1); \
								}
#else
